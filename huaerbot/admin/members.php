...
if (isset($_POST['batch_import'])) {
    ...
    if (empty($batch_data)) {
        ...
        // 支持重复导入（已存在则更新用户名和姓名）
        $stmt = $pdo->prepare("INSERT INTO users (id, username, first_name, joined_at, is_blocked) 
                               VALUES (?, ?, ?, NOW(), 0) 
        ...
        foreach ($lines as $line) {
            ...
            // 支持格式：ID,@username,姓名  或  ID,username,姓名  或  只有ID
            ...
            $username = isset($parts[1]) ? ltrim($parts[1], '@') : null;
            $first_name = $parts[2] ?? ($username ?: '未知用户');

            try {
                $stmt->execute([$id, $username ?: null, $first_name]);
                $success_count++;
            } catch (Exception $e) {
                $fail_count++;
            }
        ...
        $success = "✅ 导入完成：成功 {$success_count} 个" . ($fail_count > 0 ? "，失败 {$fail_count} 个" : "");
    }
}
