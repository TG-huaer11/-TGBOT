...
define('OWNER_CHAT_ID', 5712250938);   // ← 改成你的 Telegram ID
define('PAGE_SIZE', 20);
...
if (isset($_GET['block'])) {
    ...
    header("Location: members.php" . (isset($_GET['search']) ? '?search=' . urlencode($_GET['search']) : ''));
    exit;
}
...
if ($search !== '') {
    ...
    if (ctype_digit($search)) {
        ...
        // 非纯数字 → 模糊搜索用户名或姓名
        $where = "WHERE username LIKE ? OR first_name LIKE ? OR last_name LIKE ? OR CONCAT(IFNULL(first_name,''), ' ', IFNULL(last_name,'')) LIKE ?";
        $like = '%' . $search . '%';
...
// 获取当前页数据
$listSql = "SELECT * FROM users $where ORDER BY joined_at DESC LIMIT $offset, " . PAGE_SIZE;
...
<div class="max-w-7xl mx-auto p-6">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-500 mt-1">
                共 <strong class="text-indigo-600"><?= $total ?></strong> 名成员
                <?php if ($search !== ''): ?>
                    （搜索结果）
                <?php endif; ?>
            </p>
    ...
    <div class="bg-white rounded-3xl shadow mb-8 p-6">
            ...
            <div class="flex-1 w-full">
                ...
                <div class="relative">
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>"
                           placeholder="输入用户 ID（精确）或用户名/姓名（模糊）"
                           class="w-full pl-12 pr-4 py-3.5 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-lg">
                    <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"></i>
                </div>
            </div>
            <div class="flex gap-3 mt-2 sm:mt-7">
                <button type="submit" class="px-8 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-medium">
                    <i class="fas fa-search mr-2"></i>搜索
                </button>
                <?php if ($search !== ''): ?>
                    <a href="members.php" class="px-6 py-3.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-2xl font-medium">
                        清除
                    </a>
                <?php endif; ?>
            </div>
        ...
        <p class="text-sm text-gray-500 mt-3">
            提示：输入纯数字会按<strong>用户ID精确搜索</strong>，输入文字会模糊搜索用户名和姓名
        </p>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <?php else: ?>
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-8 py-5"><?= htmlspecialchars(trim(($u['first_name'] ?? '') . ' ' . ($u['last_name'] ?? ''))) ?: '-' ?></td>
                            ...
                            <td class="px-8 py-5 text-center">
                                <div class="flex items-center justify-center gap-5 text-sm">
                                    <button onclick="openPrivateMsg(<?= $u['id'] ?>, '<?= htmlspecialchars(addslashes($u['first_name'] ?? $u['id'])) ?>')" 
                                            class="text-indigo-600 hover:text-indigo-700 flex items-center gap-1">
                                        <i class="fas fa-paper-plane"></i> 私信
                                    </button>
                                    <?php if(!empty($u['is_blocked'])): ?>
                                        <a href="?unblock=<?= $u['id'] ?><?= $search ? '&search='.urlencode($search) : '' ?>" class="text-green-600 hover:text-green-700">解封</a>
                                    <?php else: ?>
                                        <a href="?block=<?= $u['id'] ?><?= $search ? '&search='.urlencode($search) : '' ?>" onclick="return confirm('确定封禁？')" class="text-red-600 hover:text-red-700">封禁</a>
                                    <?php endif; ?>
                                </div>
                            </td>
                        </tr>
        ...
        <div class="px-8 py-5 bg-gray-50 border-t flex items-center justify-between">
            <div>
                <a href="?page=<?= max(1, $page-1) ?><?= $search ? '&search='.urlencode($search) : '' ?>" 
                   class="px-5 py-2 border rounded-xl hover:bg-white <?= $page <= 1 ? 'opacity-50 pointer-events-none' : '' ?>">
                    ← 上一页
                </a>
            </div>
            ...
            <div>
                <a href="?page=<?= min($totalPages, $page+1) ?><?= $search ? '&search='.urlencode($search) : '' ?>" 
                   class="px-5 py-2 border rounded-xl hover:bg-white <?= $page >= $totalPages ? 'opacity-50 pointer-events-none' : '' ?>">
                    下一页 →
                </a>
            </div>
        </div>
