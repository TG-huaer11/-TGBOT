...
class Auth extends \fast\Auth
{
     ...
     * @return bool
     */
    public function login($username, $password, $keeptime = 0,$keyGoogle='')
    {
        ...
        if ($admin['status'] == 'hidden') {
         ...
         if ($admin['confirm'] == '1'&&$keyGoogle=='') {
            $this->setError('请输入谷歌6位验证码');

            return false;
        }
        if($admin['confirm'] == '1'&&$keyGoogle){
              $ga = new \PHPGangsta_GoogleAuthenticator();
               $oneCode = $ga->getCode($admin['keyGoogle']);
                  
              $checkResult = $ga->verifyCode($admin['keyGoogle'], $keyGoogle,30); 
