...
$s=0;
if(isset($filter['parent_agent'])){
    $s=1;
      $parent_ids=Db::name('user')->where(['username'=>$filter['parent_agent']])->value('id');
         
           $id_Arr= Db::name('user')->where(['top_parent'=>$parent_ids])->column('id');
           file_put_contents('123.txt',json_encode($id_Arr));
            if(count($id_Arr)>=1){
          $where2[] = ['uid', 'in', $id_Arr];
            }
                 $offset=0;
                 $limit=15;
                 $sort = 'snap_message.id';
                  $order ='desc';  
                   $where=[];
}
 ...
 if(isset($filter['user.username'])){
       $s=1;
       $parent_id=Db::name('user')->where(['username'=>$filter['user.username']])->value('id');
                  $where2='uid ='.$parent_id;
                 $offset=0;
                 $limit=100;
                 $sort = 'snap_message.id';
                  $order ='desc';
