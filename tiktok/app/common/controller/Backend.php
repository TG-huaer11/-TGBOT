...
//                    if($url=='/agent'){
//
//                    } $this->redirect('agent/index/login', [], 302, ['referer' => $url]);
//                    exit;
                if ($url == '/') {
        ...
        // 判断是否为超管
        $this->isSuperAdmin = in_array('*', $this->auth->getRuleIds($this->auth->id)) ? true : false;
        ...
        // 语言检测
//        $this->lang = $lang = strip_tags(Lang::getLangSet());
        ...
        $this->assignconfig('isSuperAdmin', $this->isSuperAdmin);
        $this->assignconfig('user_id', $this->auth->user_id);
        $agent_list= Db::name('admin')->where('user_id > 0')->select();
 $this->assign('agent_list',  $agent_list);
    }


 protected function returnUid($lang, $name)
    {
    ...
    protected function loadlang($lang, $name)
    {
        ...
        Lang::load(app()->getAppPath() . 'lang/' . $lang . '/' . $path . '.php');
    }
     ...
     * @return array
     */
    protected function buildparams($searchfields = null, $relationSearch = null, $noNeedSearch = null)
    {
...
file_put_contents('33.txt',json_encode($where));
