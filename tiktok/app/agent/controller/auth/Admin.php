    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                //创建前台用户
                $user = new User();
                $user->tel = 12345678911;
                $user->username = $params ['username'];
                $user->salt = rand(0, 99999);
                $user->create_at = time();
                $user->invite_code = (new \app\common\service\UserService)->create_invite_code();
                $user->level = 1;
                $user->parent_id = 0;
                $user->headpic = "/assets/common/img/avatar/28.8a7e137.png";
                $user->pwd = sha1('123456' . $user->salt . config('app.pwd_str'));
                $user->regip = '127.0.0.1';
                $user->salt2 = rand(0, 99999);
                $user->pwd2 = sha1('123456' . $user->salt2 . config('app.pwd_str'));
                $user->save();
    ...
    public function edit($ids = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                $adminValidate->rule([
                    'username' => ["require", "regex:\w{3,12}", "unique" => [\app\common\model\Admin::class, "username", $row->id]],
                    'email'    => ["require", "email", "unique" => [\app\common\model\Admin::class, "email", $row->id]],
                    'password' => 'regex:\S{32}',
                ]);
